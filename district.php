...
<head>
    ...
    <!-- Title Page-->
    <title>Covid-19 / District / City</title>
                ...
                <div class="section__content section__content--p30">
                    <div class="col-md-12">
                        <div class="overview-wrap">
                            <a href="chart.php" class="au-btn au-btn-icon au-btn--blue">
                                <i class="zmdi zmdi-plus"></i>Graphic</a>
                        </div>
                    </div>
                    <br>
                    <div class="container-fluid">
...
<?php
    ...
    $i=1;
    // $positive = mysqli_query($kon,"SELECT * FROM pemeriksaan WHERE hasil='Positive'");
    ...
    while($dataDistrict = mysqli_fetch_assoc($district)){
        ...
        while($dataCount = mysqli_fetch_assoc($count)){
            echo "<tr>";
            echo "<td>".$i++."</td>";
            echo "<td>".$dataDistrict['kelurahan']."</td>";
            echo "<td>".$dataCount['COUNT(id_pasien)']."</td>";
            echo "</tr>";   
        }
    }
?>
...
<?php
    $i=1;
    $city = mysqli_query($kon, "SELECT DISTINCT kota FROM pasien");
    while($dataCity = mysqli_fetch_assoc($city)){
        $countCity = mysqli_query($kon, "SELECT COUNT(id_pasien) FROM pasien WHERE kota='".$dataCity['kota']."' AND id_pasien IN (SELECT id_pasien FROM pemeriksaan WHERE hasil='Positive')");
        while($dataCountCity = mysqli_fetch_assoc($countCity)){
            echo "<tr>";
            echo "<td>".$i++."</td>";
            echo "<td>".$dataCity['kota']."</td>";
            echo "<td>".$dataCountCity['COUNT(id_pasien)']."</td>";
            echo "</tr>";   
        }
    }
?>
